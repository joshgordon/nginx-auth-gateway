...
def auth_check(request):
    ...
    if cert.user.is_authorized(site):
        try:
            site_password = SitePassword.objects.get(site=site, user=cert.user)
            site_password = site_password.password
        except SitePassword.DoesNotExist:
            site_password = ""
        ...
        response["auth_b64"] = base64.b64encode(
            (cert.user.username + ":" + site_password).encode("UTF-8")
        ).decode("UTF-8")
        response["authenticated"] = "false"
        response["authorized"] = "false"
        return response
